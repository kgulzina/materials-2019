...
ames_weather <- read_csv("../data/Ames_weather_2017-2018.csv") %>%
  ...
  group_by(day) %>%
  summarize(temp_low = min(temp_low), temp_hi = max(temp_hi))

ggplot(data = ames_weather) + 
  geom_ribbon(aes(x = day, ymin = temp_low, ymax = temp_hi))
...
perday %>%
  ...
  filter(year <= 2018) %>%
  left_join(ames_weather) %>%
  mutate(below_freezing = temp_low <=32) %>%
  ggplot(aes(x = yday, y = n)) +
  geom_vline(data = schedule, aes(xintercept = yday, colour = home)) +
  geom_vline(data = schedule17, aes(xintercept = yday, colour = Home)) +
  geom_point(aes(color = below_freezing), shape = 20) +
  scale_color_manual(values = c("Away" = "grey", "FALSE" = "orange", "Home" = "red", "TRUE" = "blue")) + 
  facet_grid(year~.) +
  theme_bw() +
  ylab("Number of police reports") +
  xlab("Day of the year")
